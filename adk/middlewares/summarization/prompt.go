...
var PromptLanguage Language = LangEnglish

func getSystemPrompt() string {
	if PromptLanguage == LangChinese {
...
func getTranscriptPathInstruction() string {
	if PromptLanguage == LangChinese {
		return transcriptPathInstructionZh
	}
...
func getFallbackUserMessagesInstruction() string {
	if PromptLanguage == LangChinese {
		return fallbackUserMessagesInstructionZh
	}
...
func getTruncatedMarkerFormat() string {
	if PromptLanguage == LangChinese {
		return truncatedMarkerFormatZh
	}
...
const fallbackUserMessagesInstructionZh = `除了上述总结外，你还应参考以下用户最近的原始消息：\n%s`
